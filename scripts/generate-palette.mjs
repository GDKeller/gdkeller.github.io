...
// ── Tailwind v4 default palette (hex values match v4 defaults) ───────
const palette = {
  slate: {
  ...
  pink: {
    50:'#fdf2f8',100:'#fce7f1',200:'#fbcfe8',300:'#f9a8d4',400:'#f472b6',
...
// Families classified as "green" vs "accent" for split-shift themes
const greenFamilies = new Set(['green', 'emerald', 'teal', 'lime', 'cyan']);

// shift: uniform number OR { greens, accents } for split shifts
const themes = {
  jade:      { shift: -8,  desc: 'Warm shift — richer greens, hotter accents' },
...
for (const [name, { shift, desc }] of Object.entries(themes)) {
  const lines = [];
  for (const [family, shades] of Object.entries(palette)) {
    // Resolve per-family shift amount
    const amt = typeof shift === 'number'
      ? shift
      : greenFamilies.has(family) ? shift.greens : shift.accents;
    lines.push(`  /* ${family} */`);
    for (const step of steps) {
      const [L, C, H] = hexToOklch(shades[step]);
      const shiftedH = ((H + amt) % 360 + 360) % 360;
      lines.push(`  --color-${family}-${step}: ${fmtOklch(L, C, shiftedH)};`);
    }
    lines.push('');
  }
  const shiftLabel = typeof shift === 'number'
    ? `${shift > 0 ? '+' : ''}${shift}°`
    : `greens ${shift.greens > 0 ? '+' : ''}${shift.greens}° / accents ${shift.accents > 0 ? '+' : ''}${shift.accents}°`;
  blocks.push(`/* ${name} — ${desc} (${shiftLabel}) */\n.theme-${name} {\n${lines.join('\n')}}`);
}

console.log(`/* Theme palette overrides — all hues shifted in OKLCH space */\n/* Generated by: node scripts/generate-palette.mjs */\n\n${blocks.join('\n\n')}`);
